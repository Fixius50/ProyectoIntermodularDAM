...
Usaremos el editor Antigravity. En el chat de la IA, en los 3 puntos de la zona del chat (arriba) le damos a habilitar los MCPs.
...
### 1\. El Ciclo de Vida del Jugador
...
Supabase como nube de datos aislados en el servidor, el cual usa PostgreSQL.  
SQLite en la app local.

## **Manejo de Datos on-server (Java \+ Spring Framework)**

El servidor es el árbitro. No confía en el cliente. Recibe las coordenadas, verifica si realmente hay un pájaro ahí, calcula el RNG (probabilidad) y guarda el estado en Supabase.

| Funcionalidad / Módulo | Tecnología / Librería | ¿Por qué se implementa? | ¿Cómo se implementa? |
| ----- | ----- | ----- | ----- |
| **API REST (Endpoints core)** | **Spring WebFlux** | Es la versión reactiva de Spring. Soporta miles de conexiones simultáneas (ideal para juegos) usando muy poca memoria. | Creando clases @RestController que devuelven Mono\<T\> o Flux\<T\> en lugar de objetos directos, procesando las peticiones de forma asíncrona. |
| **Tiempo Real (Batallas)** | **Spring RSocket / WebSockets** | Maneja el tráfico rápido de la "Arena". RSocket es más avanzado que WebSockets porque soporta control de flujo (backpressure). | Usando @MessageMapping("battle.attack") para recibir los comandos del móvil de forma continua y enviar el estado de la partida. |
| **Base de Datos (Lógica)** | **Spring Data R2DBC** | Es el driver reactivo para bases de datos relacionales. A diferencia de JPA/Hibernate, no bloquea el hilo al consultar la base de datos. | Creando interfaces R2dbcRepository\<BirdCard, UUID\> para hacer CRUDs (Create, Read, Update, Delete) asíncronos en Supabase. |
| **Base de Datos (Persistencia)** | **Supabase (PostgreSQL)** | Es una BD robusta, alojada en la nube y lista para producción. Al usarla solo desde el backend, mantienes toda la seguridad. | Se añade el driver r2dbc-postgresql al pom.xml y se configura la URL de Supabase en el application.yml de Spring Boot. |
| **Seguridad y Autenticación** | **Spring Security \+ JWT** | Valida que quien hace la petición es un jugador legítimo. Protege los endpoints contra accesos no autorizados. | Se implementa un filtro AuthenticationWebFilter que intercepta cada petición, extrae el token del header HTTP, lo valida y establece el contexto. |
| **Sistemas de Colas / Matchmaking** | **RabbitMQ (Spring AMQP)** | Vital para emparejar jugadores (Arena). Si 1000 jugadores buscan partida, RabbitMQ encola las peticiones ordenadamente para emparejarlos. | Se envía un mensaje con el nivel del jugador a una cola matchmaking\_queue usando RabbitTemplate. Un listener las procesa y crea la sesión de batalla. |
| **Caché (Rendimiento extremo)** | **Redis (Spring Data Redis)** | Almacena datos de lectura constante que no cambian rápido (ej. Estadísticas base de los pájaros, Leaderboards) para no saturar Supabase. | Se anotan métodos con @Cacheable("bird\_stats"). Spring buscará primero en memoria (Redis) y, si no está, irá a la base de datos PostgreSQL. |

## 

## **Manejo de Datos on-client (Java \+ Android)**

La app de los usuarios estará formada por Java con Android junto con servicios integrados de android y librerías de la comunidad.

| Funcionalidad / Módulo | Tecnología / Librería | ¿Por qué se implementa? | ¿Cómo se implementa? |
| :---- | :---- | :---- | :---- |
| **Comunicaciones HTTP (REST)** | **Retrofit2 \+ Gson** | Facilita enormemente el consumo de las APIs de tu servidor Spring Boot. Convierte automáticamente el JSON en objetos Java. | Se crea una interfaz Java (ej. InventoryApi) con anotaciones como @GET("/api/inventory") y Retrofit genera el código de red por ti. |
| **Asincronía y Programación Reactiva** | **RxJava 3** | Como Java no tiene "Corrutinas" (propias de Kotlin), RxJava es vital para ejecutar tareas pesadas (red, base de datos) sin congelar la app. | Usando Observable o Single para envolver llamadas de red y suscribiéndose a ellas en el hilo principal (AndroidSchedulers.mainThread()). |
| **Tiempo Real (Batallas, Chat)** | **OkHttp WebSockets** (o RSocket-Java) | Necesario para la acción multijugador. Mantiene una conexión viva bidireccional con Spring Boot para enviar/recibir ataques al instante. | Se instancia un WebSocketListener, definiendo métodos como onMessage() para actualizar la barra de vida cuando el servidor notifica un daño. |
| **Geolocalización (Avistamientos)** | **Google Play Services Location** | Accede al GPS del móvil con alta precisión y bajo consumo de batería para el mapa del juego (estilo Pokémon GO). | Se solicita permiso de ubicación al usuario y se configura un FusedLocationProviderClient para recibir actualizaciones de latitud/longitud. |
| **Almacenamiento Local (Offline)** | **Room Database** | Es un ORM oficial de Google sobre SQLite. Permite guardar el inventario localmente para que la app cargue rápido incluso con mala conexión. | Se definen entidades Java con @Entity, un DAO con @Query y Room genera la base de datos interna. Se integra perfectamente con RxJava. |
| **Inyección de Dependencias** | **Dagger / Hilt** | Desacopla los componentes (la red no debe estar pegada a la UI). Facilita las pruebas y hace que la app sea escalable. | Se anotan las clases con @Inject y @Module para que Hilt provea automáticamente instancias (ej. entregar Retrofit a todos los Fragments). |
| **Tareas en Segundo Plano** | **WorkManager** | Garantiza que ciertas tareas (ej. subir registros de batallas offline, notificaciones locales) se ejecuten aunque la app esté cerrada. | Se hereda de la clase Worker, se programa la tarea de sincronización, y Android decide el mejor momento para ejecutarla según la batería. |
| **Seguridad (Auth)** | **EncryptedSharedPreferences** | Los tokens JWT devueltos por Spring Boot deben guardarse de forma segura para que otras apps o hackers no los roben. | Se usa esta librería de AndroidX Security para cifrar el token automáticamente antes de escribirlo en el disco del móvil. |

### 

### **El flujo exacto de un "Avistamiento" (Ejemplo práctico):**

1. **Cliente (Android):** La API de *Google Location* detecta la posición. Se dispara un evento.  
2. **Cliente (Android):** Usa *Retrofit* para hacer un POST a /api/expedition/capture enviando Lat/Lon. (Protegido por el JWT en *EncryptedSharedPreferences*).  
3. **Servidor (Spring):** *Spring WebFlux* recibe la petición.  
4. **Servidor (Spring):** Valida la lógica: ¿Hay un pájaro en esa Lat/Lon? Calcula la probabilidad de captura.  
5. **Servidor (Spring):** Si se captura, usa *R2DBC* para insertar el nuevo pájaro en el inventario del jugador en *Supabase (PostgreSQL)*.  
6. **Servidor (Spring):** Responde al móvil con un Mono\<CaptureResult\> (Éxito o Fracaso).  
7. **Cliente (Android):** *RxJava* recibe la respuesta, actualiza *Room* (para acceso offline) y actualiza la *UI (XML)* mostrando la animación de captura al jugador.

## **Dilema general para el socket cliente-servidor**

El cliente podría necesitar tener instalado una app de terceros llamada Tailscale para poder acceder, cosa que desfavorecería la experiencia del usuario al depender de eso. Entonces la solución sería gestionar una red privada real (una Darknet o Tailnet) donde el APK de Android se conecta de forma directa y segura al servidor de Spring Boot, sin pasar por servidores públicos ni proxies inversos en la nube.

Para lograrlo, vamos a encapsular un nodo de Tailscale dentro de la propia app de Android usando **tsnet** (la librería de Tailscale para integrar su red en otras aplicaciones) y **gomobile** (para convertir el código de Go a una librería nativa de Android).

Aquí tienes la guía técnica paso a paso para construir este túnel cliente-servidor nativo.

### **Fase 1: El Puente en Go (Tailscale tsnet)**

Como Tailscale está escrito en el lenguaje Go, no podemos llamarlo directamente desde Java. Necesitamos escribir un pequeño "puente" en Go que inicie tsnet y cree un **servidor Proxy Local (SOCKS5)** interno en el móvil. Así, tu código Java simplemente enviará su tráfico a ese proxy local, y Go se encargará de encriptarlo y mandarlo por Tailscale.

1. Instala **Go** en tu ordenador.  
2. Crea una carpeta llamada tailscale-bridge, entra en ella y ejecuta: go mod init tailscalebridge  
3. Descarga las dependencias: go get tailscale.com/tsnet y un servidor socks5 de Go.  
4. Crea un archivo bridge.go con este diseño conceptual:
...
import (  
	"log"  
	"tailscale.com/tsnet"  
	// Librería estándar de socks5 en Go  
	"github.com/armon/go-socks5"   
)

var tsServer \*tsnet.Server

// StartTailscale inicia el túnel de Tailscale y abre un proxy SOCKS5 en localhost:1080  
func StartTailscale(authKey string) {  
	tsServer \= \&tsnet.Server{  
		Hostname: "android-client-app",  
		AuthKey:  authKey, // Clave generada en el panel de Tailscale  
	}

	// Iniciamos el nodo de Tailscale en segundo plano  
	err := tsServer.Start()  
	if err \!= nil {  
		log.Fatal(err)  
	}

	// Configuramos el Proxy SOCKS5 para que use la red de Tailscale  
	conf := \&socks5.Config{  
		Dial: tsServer.Dial, // ¡La magia ocurre aquí\! Todo el tráfico sale por Tailscale  
	}  
	server, \_ := socks5.New(conf)

	// Iniciamos el proxy SOCKS5 localmente en el móvil  
	go server.ListenAndServe("tcp", "127.0.0.1:1080")  
}
...
### **Fase 2: Compilar el .AAR para Android**

Ahora vamos a convertir ese código de Go en una librería nativa de Java/Android.

1. Instala gomobile:

go install [golang.org/x/mobile/cmd/gomobile@latest](http://golang.org/x/mobile/cmd/gomobile@latest); gomobile init

2. Compila el puente para Android (esto generará un archivo tailscalebridge.aar):

gomobile bind \-target=android \-androidapi 24 ./

### **Fase 3: Integración en el Cliente Android (Java)**

Ahora tomamos ese archivo .aar y lo metemos en nuestro Android Studio.

1. Copia tailscalebridge.aar a la carpeta app/libs de tu proyecto Android.  
2. En tu build.gradle (del módulo app), añádelo a las dependencias:  
3. Groovy: implementation files('libs/tailscalebridge.aar')

**Iniciando el túnel en tu Código Java:**

En tu clase principal (por ejemplo MainActivity.java o en una clase que herede de Application), inicias el puente de Tailscale usando una **Auth Key Epímera** (se generan en el panel de administración de Tailscale, marcalas como *Reusable* y *Ephemeral* para que los dispositivos se borren solos al desconectarse).

import tailscalebridge.Tailscalebridge; // Importado del .AAR generado
...
public class GameApplication extends Application {  
    ...
    public void onCreate() {  
        ...
        // Inicia el nodo invisible de Tailscale en segundo plano  
        // Esto levanta el proxy SOCKS5 en 127.0.0.1:1080  
        String authKey \= "tskey-auth-XXXXXXXXXXXX-YYYYYYYYYYYYYYYYY";  
        Tailscalebridge.startTailscale(authKey);  
    }  
}

**Configurando Retrofit / OkHttp (Java):**

Aquí está el truco maestro. Le decimos a OkHttp (la librería que usa Retrofit y WebSockets) que todo su tráfico debe pasar por el Proxy SOCKS5 local que acaba de levantar Go.
...
// 1\. Apuntamos al proxy interno que está conectado a Tailscale  
Proxy tailscaleProxy \= new Proxy(Proxy.Type.SOCKS, new InetSocketAddress("127.0.0.1", 1080));

// 2\. Construimos el cliente inyectando el proxy  
OkHttpClient client \= new OkHttpClient.Builder()  
        .proxy(tailscaleProxy)  
        .build();

// 3\. Configuramos Retrofit para apuntar a la IP PRIVADA de tu Spring Boot en Tailscale  
Retrofit retrofit \= new Retrofit.Builder()  
        .client(client)  
        .baseUrl("http://100.95.x.x:8080") // La IP de Tailscale de tu PC/Servidor  
        .build();

### **Fase 4: El Servidor Spring Boot**

Del lado de tu servidor Spring Boot, **no tienes que cambiar absolutamente nada de código**.

Simplemente asegúrate de que la máquina donde corres el .jar de Spring Boot tenga el cliente normal de Tailscale instalado y esté en la misma red que la clave que usaste en la app.

### **Consideraciones clave para este enfoque:**

* **Gestión del ciclo de vida:** Asegúrate de llamar a Tailscalebridge.stopTailscale() cuando la aplicación se cierre o el usuario la minimice por completo para no agotar la batería del móvil, ya que mantener el túnel WireGuard activo consume recursos.  
* **Permisos en Android:** Tu app necesitará el permiso estándar \<uses-permission android:name="android.permission.INTERNET" /\> en el AndroidManifest.xml, pero **no** requerirá el permiso especial de VPN (VpnService), por lo que para el usuario la conexión será completamente invisible.
...
* **Framework Core:** Java 21 con Spring Boot 4.0 (WebFlux / Netty).  
* **Comunicación en Tiempo Real:** RSocket (Flujo bidireccional TCP).  
* **Base de Datos Principal:** Supabase (PostgreSQL) mediante Spring Data R2DBC.  
* **Almacenamiento Multimedia:** Supabase Storage (Bucket público).  
* **Infraestructura de Red:** Tailscale VPN (WireGuard) para evasión de NAT.

## **2\. Preparación del Entorno de Desarrollo (Lubuntu)**

Para desarrollar el servidor backend y ejecutar las herramientas del equipo, es necesario instalar las siguientes dependencias base:

### **2.1. Herramientas Base, Python y Control de Versiones**

**\# Actualizar repositorios e instalar Python (para herramientas internas)**
...
**\# Control de Versiones y Git LFS (Para modelos IA y assets)**
...
Para interactuar con el repositorio oficial sin bloqueos de autenticación:
...
## **3\. Herramienta Interna: AVIS Dev Uploader (Pro Edition)**

Aplicación de escritorio (GUI) en Python para sincronizar archivos al repositorio esquivando el límite de 100MB de GitHub. Detecta extensiones pesadas (`.gguf`, `.psd`, `.mp4`) y ejecuta automáticamente `git lfs track`.

**Instalación:**

mkdir \~/avis-dev-tools && cd \~/avis-dev-tools
...
**Código Fuente (`app.py`):** *(El código completo de la interfaz de usuario se encuentra en la documentación interna de herramientas del equipo. Configurado para procesar extensiones pesadas de forma automática y hacer push con un solo clic).*

## **4\. Infraestructura de Red: VPN Peer-to-Peer (Tailscale)**

Para garantizar la comunicación directa entre los clientes Android y el servidor local (Lubuntu) saltando los firewalls institucionales.

### **4.1. Script de Autoconfiguración (`auth_tailscale.sh`)**

Ubicado en `/home/lubuntu/auth_tailscale.sh`.

\#\!/bin/bash

TAILSCALE\_AUTH\_KEY="tskey-auth-k4cntXLUiW11CNTRL-ZriLhcmNRY6WHVTap15NZ6ygQTDoJY4o"

echo "Autenticando el servidor en la Tailnet de AVIS..."

sudo tailscale up \--authkey=${TAILSCALE\_AUTH\_KEY}

sudo systemctl enable \--now tailscaled

echo "IP virtual estática: $(tailscale ip \-4)"

### **4.2. Resiliencia y Autoarranque (Crontab)**

Para garantizar que el túnel VPN se levante siempre tras un corte de luz:

sudo crontab \-e

**\# Añadir al final del archivo:**

@reboot /home/lubuntu/auth\_tailscale.sh

## **5\. Servidor Backend: Spring Boot 4 (WebFlux) & RSocket**

### **5.1. Generación del Proyecto Base**
...
### **5.2. Configuración (`src/main/resources/application.properties`)**
...
**\# Conexión Reactiva a Supabase (PostgreSQL)**

spring.r2dbc.url=r2dbc:postgresql://db.shmutxsmjokamnxrkufe.supabase.co:5432/postgres

spring.r2dbc.username=postgres

spring.r2dbc.password=${DB\_PASSWORD}

### **5.3. Despliegue como Servicio Nativo (Systemd)**

Garantiza la alta disponibilidad y arranque condicionado a la red.

Archivo (`/etc/systemd/system/avis-server.service`):
...
## **6\. Base de Datos (Supabase) y Persistencia Reactiva**

### **6.1. Esquema Relacional (SQL)**

Ejecutado en el clúster PostgreSQL de Supabase.
...
CREATE TABLE bird\_cards (
    ...
    attack\_damage INT NOT NULL DEFAULT 0,   
...
CREATE TABLE player\_inventory (

    id UUID PRIMARY KEY DEFAULT gen\_random\_uuid(),

    player\_id UUID REFERENCES players(id) ON DELETE CASCADE,

    bird\_card\_id UUID REFERENCES bird\_cards(id) ON DELETE CASCADE,

    captured\_lat DOUBLE PRECISION,          

    captured\_lon DOUBLE PRECISION,          

    captured\_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL

);

### **6.2. Mapeo Objeto-Relacional en Java (Entities & Repositories)**

Uso de `R2dbcRepository` y `Lombok` para consultas asíncronas no bloqueantes.

**Ejemplo Entidad (`BirdCard.java`):**
...
**Repositorios Reactivos (`GameRepositories.java`):**

Java

package com.avis.server.repository;

import com.avis.server.model.\*;
