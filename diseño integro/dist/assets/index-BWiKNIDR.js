var J=Object.defineProperty;var Z=(s,e,a)=>e in s?J(s,e,{enumerable:!0,configurable:!0,writable:!0,value:a}):s[e]=a;var N=(s,e,a)=>Z(s,typeof e!="symbol"?e+"":e,a);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const u of document.querySelectorAll('link[rel="modulepreload"]'))m(u);new MutationObserver(u=>{for(const g of u)if(g.type==="childList")for(const l of g.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&m(l)}).observe(document,{childList:!0,subtree:!0});function a(u){const g={};return u.integrity&&(g.integrity=u.integrity),u.referrerPolicy&&(g.referrerPolicy=u.referrerPolicy),u.crossOrigin==="use-credentials"?g.credentials="include":u.crossOrigin==="anonymous"?g.credentials="omit":g.credentials="same-origin",g}function m(u){if(u.ep)return;u.ep=!0;const g=a(u);fetch(u.href,g)}})();class K{constructor(e){N(this,"container");N(this,"screens",new Map);this.container=document.getElementById(e)}register(e,a){this.screens.set(e,a)}async navigate(e){const a=this.screens.get(e);a&&(this.container.innerHTML="",await a(),window.location.hash=e)}init(){window.addEventListener("hashchange",()=>{const a=window.location.hash.replace("#","");this.screens.has(a)&&this.navigate(a)});const e=window.location.hash.replace("#","")||"home";this.navigate(e)}}const T=s=>`
    <header class="sticky top-0 z-50 px-6 py-4 lg:px-12 flex items-center justify-between glass-panel mt-4 mx-4 rounded-xl shadow-sm">
        ...
        <nav class="hidden md:flex items-center gap-8">
            ${[{id:"home",label:"El Santuario",icon:"home"},{id:"arena",label:"El Certamen",icon:"swords"},{id:"expedition",label:"La Expedición",icon:"explore"},{id:"social",label:"Social",icon:"group"},{id:"store",label:"Tienda",icon:"shopping_cart"}].map(m=>{const u=m.id===s;return`
            <a class="nav-link cursor-pointer text-sm flex items-center gap-2 ${u?"text-primary font-bold":"text-slate-600 dark:text-slate-300 font-medium hover:text-primary transition-colors"} ${u?"border-b-2 border-primary pb-0.5":""}" data-screen="${m.id}">
    ...
    <div class="relative z-10 flex flex-col flex-grow w-full max-w-[1440px] mx-auto h-screen">
        ...
        <main class="flex-grow flex flex-col lg:flex-row gap-6 p-4 lg:p-8 overflow-hidden">
            ...
            <div class="flex-grow relative bg-slate-200 dark:bg-slate-900 rounded-[3rem] overflow-hidden shadow-2xl border-8 border-white dark:border-slate-800 flex flex-col group min-h-[400px]">
                ...
                <!-- Leaflet Container -->
                <div id="map-canvas" class="absolute inset-0 z-10 transition-all duration-1000 ${A?"blur-sm":""}"></div>

                <!-- Time Phase Filter Overlay -->
                <div class="absolute inset-0 z-20 pointer-events-none transition-colors duration-1000 ${l.phase==="Night"?"bg-indigo-950/40 mix-blend-multiply":l.phase==="Afternoon"?"bg-orange-500/10 mix-blend-soft-light":"bg-transparent"}"></div>
                ...
                <div class="absolute top-0 left-0 right-0 p-8 z-30 flex justify-between items-start pointer-events-none">
                    <div class="pointer-events-auto">
                        <div class="glass-panel px-4 py-1.5 rounded-full flex items-center gap-2 mb-2 w-fit">
                            <span class="w-3 h-3 rounded-full ${l.phase==="Night"?"bg-indigo-400":"bg-primary"} animate-pulse shadow-[0_0_8px_currentColor]"></span>
                            <span class="text-[10px] font-black uppercase tracking-widest">${l.phase} en Pinto</span>
                        </div>
                ...
                <div class="absolute bottom-10 left-1/2 -translate-x-1/2 z-50">
                    <button id="btn-scan" class="group bg-primary hover:bg-primary-dark text-slate-900 px-8 py-4 rounded-2xl font-black uppercase tracking-widest shadow-2xl transition-all hover:scale-105 active:scale-95 flex items-center gap-3 disabled:opacity-50" ${A?"disabled":""}>
                        <span class="material-symbols-outlined text-xl ${A?"animate-spin":""}">radar</span>
                        ${A?"Escaneando...":"Escanear Entorno"}
                    </button>
                </div>
            </div>
...
<style>
    ...
    .landmark-label {
        ...
        `,z(s),a(),g()},a=()=>{const{time:l}=v.getState(),t=l.phase==="Night"?"https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png":"https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png";_=L.map("map-canvas",{zoomControl:!1,attributionControl:!1}).setView([40.242,-3.7],15),L.tileLayer(t,{maxZoom:19}).addTo(_),L.marker([40.245,-3.698],{icon:L.divIcon({className:"landmark-label",html:'<div class="bg-white/80 backdrop-blur px-2 py-1 rounded-full border border-white shadow-sm text-[8px] font-black uppercase whitespace-nowrap">Parque Cabeza de Hierro</div>',iconAnchor:[60,10]})}).addTo(_),L.marker([40.242,-3.7],{icon:L.divIcon({className:"landmark-label",html:'<div class="bg-white/80 backdrop-blur px-2 py-1 rounded-full border border-white shadow-sm text-[8px] font-black uppercase whitespace-nowrap">Torre de Éboli</div>',iconAnchor:[40,10]})}).addTo(_),L.marker([40.241,-3.699],{icon:L.divIcon({className:"landmark-label",html:'<div class="bg-white/80 backdrop-blur px-2 py-1 rounded-full border border-white shadow-sm text-[8px] font-black uppercase whitespace-nowrap">Iglesia Sto. Domingo</div>',iconAnchor:[50,10]})}).addTo(_),m()},m=()=>{if(!_)return;const{time:l,playerBirds:t}=v.getState();D.filter(d=>{const r=d.preferredPhase.includes(l.phase),i=t.some(x=>x.id===d.id);return r&&!i}).filter(d=>R.includes(d.id)).forEach(d=>{L.marker([d.lat,d.lng],{icon:L.divIcon({className:"bird-marker",html:`
                    ...
                    `,iconSize:[56,56],iconAnchor:[28,28]})}).addTo(_).on("click",()=>u(d))})},u=l=>{const t=document.getElementById("study-modal"),o=document.getElementById("study-modal-content"),d=document.getElementById("modal-bird-image"),r=document.getElementById("modal-bird-name"),i=document.getElementById("modal-bird-scientific"),x=document.getElementById("modal-bird-fact"),f=document.getElementById("btn-close-study");if(t&&o&&d&&r&&i&&x){d.src=l.image,r.textContent=l.name,i.textContent=l.scientificName,x.textContent=`"${l.fact}"`,t.classList.remove("hidden"),t.classList.add("flex"),setTimeout(()=>{o.classList.remove("scale-0"),o.classList.add("scale-100"),d.classList.remove("grayscale")},50);const k=()=>{const{playerBirds:C,time:b,weather:p}=v.getState(),c={...l,isStudied:!0,xp:250};v.setState({playerBirds:[...C,c],activeBirdsCount:v.getState().activeBirdsCount+1});const n=document.getElementById("diary-entries");if(n){const h=document.createElement("div");h.className="animate-fade-in-down bg-white/50 dark:bg-slate-700/50 p-4 rounded-3xl border border-white dark:border-slate-600 shadow-sm relative overflow-hidden group mb-4",h.innerHTML=`
                        <div class="flex gap-4">
                            <div class="w-16 h-16 rounded-2xl bg-cover bg-center shrink-0 border-2 border-white shadow-md" style="background-image: url('${l.image}')"></div>
                            <div class="flex-grow">
                                <div class="flex justify-between items-start">
                                    <h4 class="font-handwriting text-lg font-bold text-amber-900 dark:text-amber-100 leading-none">${l.name}</h4>
                                    <div class="flex items-center gap-1.5 opacity-50">
                                        ...
                                        <span class="text-[9px] font-bold uppercase">${b.phase}</span>
                                    </div>
                                </div>
                                <p class="text-[10px] font-bold text-amber-800/60 dark:text-slate-400 italic mt-0.5">${l.scientificName}</p>
                                <p class="text-xs text-slate-700 dark:text-slate-300 mt-2 font-handwriting leading-tight">"${l.fact}"</p>
                    ...
                    `;const w=n.querySelector(".opacity-30");w&&w.remove(),n.prepend(h)}R=R.filter(h=>h!==l.id),o.classList.remove("scale-100"),o.classList.add("scale-0"),setTimeout(()=>{t.classList.add("hidden"),t.classList.remove("flex"),e()},500),f==null||f.removeEventListener("click",k)};f==null||f.addEventListener("click",k)}},g=()=>{const l=document.getElementById("btn-scan");l==null||l.addEventListener("click",()=>{A=!0,e(),setTimeout(()=>{A=!1;const{time:t,playerBirds:o}=v.getState();R=D.filter(r=>{const i=r.preferredPhase.includes(t.phase),x=o.some(f=>f.id===r.id);return i&&!x}).map(r=>r.id),e()},2e3)})};e()},se=async s=>{const e=["¿Sabías que el Petirrojo es muy territorial? Defenderá su zona con cantos potentes incluso contra pájaros más grandes.","Los días de lluvia son ideales para ver aves acuáticas cerca de la base del Gran Roble. Busca chapoteos.","¡Mantén tu racha viva! Una racha de 5 días aumenta las probabilidades de avistamientos raros en un 10%.","Para fabricar 'Néctar de Tormenta' se requiere lluvia. Atento al widget del clima local.","Las aves rapaces son más activas al mediodía, aprovechando las corrientes de aire caliente para planear.","Escucha atentamente por la mañana: el 'Coro del Alba' es cuando la mayoría de aves cantoras marcan su territorio.","Los búhos y otras aves nocturnas tienen plumajes especiales que les permiten volar en completo silencio.","Si ves un pájaro con colores muy brillantes, suele ser un macho intentando impresionar en la época de cría.","Anota cada avistamiento en tu álbum; el conocimiento naturalista es la clave para ser un gran conservador.","¿Ves plumas en el suelo? Podrían ser de una muda reciente. Las aves cambian sus plumas para mantenerse protegidas."],a=()=>{let{playerBirds:t,activeBirdsCount:o,rareSightings:d,streak:r,weather:i,time:x,pinnedLinks:f}=v.getState(),k=!1;t=t.map(p=>{if(p.image.includes("lh3.googleusercontent")){const c=D.find(n=>n.id===p.id);if(c)return k=!0,{...p,image:c.image,audioUrl:c.audioUrl}}return p}),k&&v.setState({playerBirds:t});const C=e[Math.floor(Date.now()/(1e3*60*60*24))%e.length];(window.location.hash==="#home"||window.location.hash===""||window.location.hash==="#")&&(s.innerHTML=`
<div class="bg-cream dark:bg-background-dark text-slate-900 dark:text-slate-100 font-display transition-colors duration-200 min-h-screen flex flex-col relative overflow-x-hidden ${x.phase==="Night"?"brightness-75 grayscale-[0.2]":x.phase==="Morning"?"sepia-[0.1] saturate-[1.2]":""}">
...
<div class="flex justify-between items-start">
<div>
<p class="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">${x.phase} • ${i?i.location:"Loading..."}</p>
...
<span class="material-symbols-outlined text-xl text-slate-400">${x.icon}</span>
</div>
</div>
...
${x.phase==="Night"?"Nocturnal birds are currently appearing.":"Diurnal birds are most active now."}
</p>
</div>
<!-- Stats -->
...
<p class="text-xl font-bold text-sage-800 dark:text-white mt-0.5">${o} Perched</p>
</div>
<div class="bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300 px-2 py-1 rounded text-xs font-bold">${t.length} Found</div>
</div>
<div class="bg-white dark:bg-sage-800 p-5 rounded-2xl border border-sage-100 dark:border-sage-700 shadow-sm flex items-center justify-between">
<div>
<p class="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">Day Streak</p>
<p class="text-xl font-bold text-sage-800 dark:text-white mt-0.5">${r} Days</p>
</div>
...
`:t.map((p,c)=>`
      <div class="absolute animate-float bird-marker-container" style="top: ${20+c*15}%; left: ${30+c*20}%; animation-delay: ${c*.5}s" data-bird-id="${p.id}">
...
<div class="text-center mb-4 ${t.length===0?"opacity-0":""}">
...
`).join("")}
    ...
    `,z(s),u())},m=document.createElement("style");m.textContent=`
    ...
    `,document.head.appendChild(m);const u=()=>{var x,f,k,C,b,p;const t=document.getElementById("modal-edit-quick"),o=document.getElementById("all-screens-list"),d=[{id:"l1",screen:"expedition",label:"Expedition",icon:"explore",image:"https://images.pexels.com/photos/15286/pexels-photo.jpg?auto=compress&cs=tinysrgb&w=400",description:"Start a new journey into the wild."},{id:"l2",screen:"album",label:"The Album",icon:"menu_book",image:"https://images.pexels.com/photos/415071/pexels-photo-415071.jpeg?auto=compress&cs=tinysrgb&w=400",description:"Review your findings and lore."},{id:"l3",screen:"workshop",label:"Workshop",icon:"handyman",image:"https://images.pexels.com/photos/175039/pexels-photo-175039.jpeg?auto=compress&cs=tinysrgb&w=400",description:"Craft equipment and supplies."},{id:"l4",screen:"arena",label:"Arena",icon:"swords",image:"https://images.pexels.com/photos/1762578/pexels-photo-1762578.jpeg?auto=compress&cs=tinysrgb&w=400",description:"Test your birds against challengers."}];(x=document.getElementById("btn-edit-quick"))==null||x.addEventListener("click",()=>{if(t&&o){const{pinnedLinks:c}=v.getState();o.innerHTML=d.map(n=>{const h=c.some(w=>w.id===n.id);return`
                        <div class="flex items-center justify-between p-3 rounded-2xl bg-slate-100 dark:bg-slate-800 border border-transparent hover:border-primary/30 transition-all">
                            ...
                            <button class="btn-toggle-pin p-2 rounded-lg ${h?"text-primary":"text-slate-400"}" data-id="${n.id}">
                                <span class="material-symbols-outlined">${h?"push_pin":"keep"}</span>
                            </button>
                        </div>
                    `}).join(""),t.classList.remove("hidden"),o.querySelectorAll(".btn-toggle-pin").forEach(n=>{n.addEventListener("click",h=>{var M;const w=h.currentTarget.dataset.id,{pinnedLinks:y}=v.getState();let j;if(y.some(S=>S.id===w))j=y.filter(S=>S.id!==w);else{const S=d.find(q=>q.id===w);S&&(j=[...y,S])}j&&v.setState({pinnedLinks:j}),(M=document.getElementById("btn-edit-quick"))==null||M.click()})})}}),(f=document.getElementById("btn-close-modal"))==null||f.addEventListener("click",()=>{t==null||t.classList.add("hidden"),a()}),(k=document.getElementById("btn-save-quick"))==null||k.addEventListener("click",()=>{t==null||t.classList.add("hidden"),a()}),(C=document.getElementById("modal-overlay"))==null||C.addEventListener("click",()=>{t==null||t.classList.add("hidden"),a()});const r=document.getElementById("modal-bird-detail");let i=null;document.querySelectorAll(".bird-marker-container").forEach(c=>{c.addEventListener("click",n=>{const h=n.currentTarget.dataset.birdId,{playerBirds:w}=v.getState(),y=w.find(j=>j.id===h);if(y&&r){const j=document.getElementById("detail-bird-image"),M=document.getElementById("detail-bird-name"),S=document.getElementById("detail-bird-level"),q=document.getElementById("detail-origin-text"),O=document.getElementById("detail-xp-text"),W=document.getElementById("detail-xp-bar"),V=document.getElementById("detail-next-xp");j&&(j.style.backgroundImage=`url('${y.image}')`),M&&(M.textContent=y.name),S&&(S.textContent=`Nivel ${y.level}`),q&&(q.textContent=y.origin||"Región de Pinto"),O&&(O.textContent=`${y.xp} XP`),V&&(V.textContent=`${y.maxXp} XP`),W&&(W.style.width=`${y.xp/y.maxXp*100}%`),r.classList.remove("hidden");const H=document.getElementById("btn-play-audio"),I=document.getElementById("play-icon"),$=document.getElementById("audio-waveform");if(H&&y.audioUrl){H.replaceWith(H.cloneNode(!0));const U=document.getElementById("btn-play-audio");U==null||U.addEventListener("click",()=>{if(i&&(i.pause(),i.src===y.audioUrl)){i=null,I&&(I.textContent="play_arrow"),$==null||$.querySelectorAll("div").forEach(P=>P.classList.remove("animate-bounce-slow"));return}i=new Audio(y.audioUrl),i.play(),I&&(I.textContent="pause"),$==null||$.querySelectorAll("div").forEach((P,X)=>{P.style.animationDelay=`${X*.1}s`,P.classList.add("animate-bounce-slow")}),i.onended=()=>{I&&(I.textContent="play_arrow"),$==null||$.querySelectorAll("div").forEach(P=>P.classList.remove("animate-bounce-slow"))}})}}})}),(b=document.getElementById("btn-close-bird"))==null||b.addEventListener("click",()=>{r==null||r.classList.add("hidden"),i&&(i.pause(),i=null)}),(p=document.getElementById("modal-bird-overlay"))==null||p.addEventListener("click",()=>{r==null||r.classList.add("hidden"),i&&(i.pause(),i=null)})};a();const g=localStorage.getItem("last_login"),l=new Date().toDateString();if(g!==l){const{streak:t}=v.getState();v.setState({streak:t+1}),localStorage.setItem("last_login",l),a()}if(!v.getState().weather){const t=await ee();v.setState({weather:t}),a()}},F=s=>{let{playerBirds:e,opponentBirds:a,weather:m,time:u}=v.getState(),g=window.selectedBirdId||e[0].id,l=e.find(b=>b.id===g)||e[0];const t=a[0];let o=0,d=0,r=1;const i=b=>{const c=["canto","plumaje","vuelo"][Math.floor(Math.random()*3)];let n=l[b],h=t[c],w="none";b==="vuelo"&&c==="canto"||b==="canto"&&c==="plumaje"||b==="plumaje"&&c==="vuelo"?(n*=1.5,w="user"):(c==="vuelo"&&b==="canto"||c==="canto"&&b==="plumaje"||c==="plumaje"&&b==="vuelo")&&(h*=1.5,w="opponent");const y=(m==null?void 0:m.condition.toLowerCase())||"";return b==="vuelo"&&(y.includes("sun")||y.includes("clear"))&&(n*=1.1),b==="canto"&&u.phase==="Morning"&&(n*=1.1),b==="plumaje"&&y.includes("rain")&&(n*=1.1),{winner:n>h?"user":n<h?"opponent":"draw",userAttr:b,opponentAttr:c,userScore:n,opponentScore:h,advantage:w}},x=()=>{window.location.hash==="#arena"&&(s.innerHTML=`
<div class="bg-background-light dark:bg-background-dark font-display min-h-screen flex flex-col overflow-x-hidden text-slate-900 dark:text-slate-100 relative">
    ...
    <div class="relative z-10 flex flex-col flex-grow w-full max-w-[1440px] mx-auto">
        ...
        <main class="flex-grow flex flex-col items-center justify-start py-6 px-4 md:px-8 w-full max-w-[1200px] mx-auto z-10">
            ...
            <div class="w-full flex items-center justify-between mb-8 bg-white/80 dark:bg-background-dark/80 backdrop-blur-md p-6 rounded-3xl border border-primary/20 shadow-journal">
                <div class="flex items-center gap-4">
                    <div class="relative">
                        <img src="${l.image}" class="size-20 rounded-2xl object-cover border-4 border-primary shadow-lg">
                        <div class="absolute -bottom-2 -right-2 bg-primary text-white text-[10px] font-black px-2 py-1 rounded-lg">TÚ</div>
                    </div>
                    <div>
                        <h2 class="text-xl font-bold">${l.name}</h2>
                        <div class="flex gap-1.5 mt-2">
                            ${Array.from({length:5}).map((b,p)=>`
                                <div class="size-4 rounded-full border-2 border-primary/30 ${p<o?"bg-primary shadow-[0_0_10px_rgba(var(--primary-rgb),0.5)]":"bg-slate-100 dark:bg-slate-800"}"></div>
                            `).join("")}
                        </div>
                    </div>
                </div>
                ...
                <div class="text-center bg-sage-50 dark:bg-sage-900/40 px-8 py-3 rounded-2xl border border-sage-200 dark:border-sage-800">
                    ...
                    <p class="text-4xl font-black text-ink-dark dark:text-white leading-none">${r<=5?`R${r}`:"FIN"}</p>
                </div>
                ...
                <div class="flex items-center gap-4 text-right">
                    <div>
                        ...
                        <div class="flex gap-1.5 mt-2 justify-end">
                            ${Array.from({length:5}).map((b,p)=>`
                                <div class="size-4 rounded-full border-2 border-red-500/30 ${p<d?"bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.5)]":"bg-slate-100 dark:bg-slate-800"}"></div>
                            `).join("")}
                        </div>
                    </div>
                    <div class="relative">
                        <img src="${t.image}" class="size-20 rounded-2xl object-cover border-4 border-red-500 shadow-lg">
                ...
                <div class="lg:col-span-4 flex flex-col gap-4">
                    <div class="glass-panel p-6 rounded-[2.5rem] border border-primary/10 shadow-journal flex flex-col gap-4 h-full bg-white/90 dark:bg-sage-900/40">
                        ...
                        <button class="attr-btn group relative flex items-center justify-between p-5 bg-amber-50 dark:bg-amber-900/20 border-2 border-amber-200 dark:border-amber-800 rounded-2xl hover:border-amber-400 hover:scale-[1.02] active:scale-95 transition-all ${r>5||window.isRoundAnimating?"opacity-50 pointer-events-none":""}" data-attr="canto">
                            ...
                            <span class="text-2xl font-black text-amber-600">${l.canto}</span>
                        </button>

                        <button class="attr-btn group relative flex items-center justify-between p-5 bg-emerald-50 dark:bg-emerald-900/20 border-2 border-emerald-200 dark:border-emerald-800 rounded-2xl hover:border-emerald-400 hover:scale-[1.02] active:scale-95 transition-all ${r>5||window.isRoundAnimating?"opacity-50 pointer-events-none":""}" data-attr="plumaje">
                            <div class="flex items-center gap-4">
                            ...
                            <span class="text-2xl font-black text-emerald-600">${l.plumaje}</span>
                        </button>

                        <button class="attr-btn group relative flex items-center justify-between p-5 bg-blue-50 dark:bg-blue-900/20 border-2 border-blue-200 dark:border-blue-800 rounded-2xl hover:border-blue-400 hover:scale-[1.02] active:scale-95 transition-all ${r>5||window.isRoundAnimating?"opacity-50 pointer-events-none":""}" data-attr="vuelo">
                            <div class="flex items-center gap-4">
                            ...
                            <span class="text-2xl font-black text-blue-600">${l.vuelo}</span>
                        </button>
                        ...
                        <div id="battle-scene" class="w-full h-full flex items-center justify-center relative">
                            <div class="text-center animate-fade-in" id="vs-display">
                                ...
                                <p class="text-sm font-bold text-slate-500 uppercase tracking-widest bg-white dark:bg-background-dark px-6 py-2 rounded-full border border-slate-100 dark:border-slate-800 shadow-sm relative z-10">
                                    Elige un atributo para la Ronda ${r}
                                </p>
                            </div>
                        </div>
                    </div>
        ...
        `,z(s),f())},f=()=>{document.querySelectorAll(".attr-btn").forEach(b=>{b.addEventListener("click",async p=>{const c=p.currentTarget.dataset.attr;if(!c||window.isRoundAnimating)return;window.isRoundAnimating=!0;const n=i(c),h=document.getElementById("vs-display");if(h&&(h.innerHTML=`
                            ...
                            <div class="flex items-center justify-between gap-12 w-full">
                                <div class="flex-1 flex flex-col items-center gap-4 p-6 bg-white dark:bg-slate-800 rounded-[2rem] shadow-xl border-b-4 border-primary">
                                    ...
                                    <span class="material-symbols-outlined text-4xl text-primary">
                                        ${c==="canto"?"music_note":c==="plumaje"?"shield":"air"}
                                    </span>
                                    <p class="text-3xl font-black text-ink-dark dark:text-white">${Math.round(n.userScore)}</p>
                                    <p class="text-[10px] font-bold text-primary uppercase">${c}</p>
                                    ${n.advantage==="user"?'<span class="text-[10px] font-black text-emerald-500 animate-pulse">+50% VENTAJA</span>':""}
                                </div>
                                ...
                                <div class="flex-1 flex flex-col items-center gap-4 p-6 bg-white dark:bg-slate-800 rounded-[2rem] shadow-xl border-b-4 border-red-500">
                                    ...
                                    <span class="material-symbols-outlined text-4xl text-red-500">
                                        ${n.opponentAttr==="canto"?"music_note":n.opponentAttr==="plumaje"?"shield":"air"}
                                    </span>
                                    <p class="text-3xl font-black text-ink-dark dark:text-white">${Math.round(n.opponentScore)}</p>
                                    <p class="text-[10px] font-bold text-red-500 uppercase">${n.opponentAttr}</p>
                                    ${n.advantage==="opponent"?'<span class="text-[10px] font-black text-red-500 animate-pulse">+50% VENTAJA</span>':""}
                                </div>
                            </div>
                            <div class="animate-bounce-slow">
                                <p class="text-2xl font-black italic uppercase tracking-widert ${n.winner==="user"?"text-primary":n.winner==="opponent"?"text-red-500":"text-slate-500"}">
                                    ${n.winner==="user"?"¡Has ganado la ronda!":n.winner==="opponent"?"Ronda para el oponente":"¡Empate!"}
                                </p>
                            </div>
                        </div>
                    `),C(`R${r}: <span class="font-bold text-primary">${c.toUpperCase()}</span> (${Math.round(n.userScore)}) vs <span class="font-bold text-red-500">${n.opponentAttr.toUpperCase()}</span> (${Math.round(n.opponentScore)})`),await new Promise(w=>setTimeout(w,2500)),n.winner==="user"?o++:n.winner==="opponent"&&d++,r++,window.isRoundAnimating=!1,r>5){const w=o>d?"user":o<d?"opponent":"draw";k(w)}else x()})})},k=b=>{var c;const p=document.getElementById("vs-display");p&&(p.innerHTML=`
                <div class="animate-scale-up text-center bg-white dark:bg-slate-800 p-12 rounded-[3rem] shadow-2xl border-2 border-primary/20 max-w-md w-full">
                    <div class="size-20 bg-primary/10 rounded-full flex items-center justify-center mx-auto mb-6">
                        <span class="material-symbols-outlined text-5xl text-primary">${b==="user"?"workspace_premium":"potted_plant"}</span>
                    </div>
                    <h2 class="text-5xl font-black uppercase mb-2 tracking-tighter text-ink-dark dark:text-white">
                        ${b==="user"?"¡VICTORIA!":b==="opponent"?"FIN DEL DUELO":"EMPATE"}
                    </h2>
                    <p class="text-slate-500 font-bold mb-8">Marcador Final: ${o} - ${d}</p>
            ...
            `,(c=document.getElementById("btn-restart-arena"))==null||c.addEventListener("click",()=>{o=0,d=0,r=1,F(s)}))},C=b=>{const p=document.getElementById("log-entries");if(p){const c=document.createElement("div");c.className="text-[11px] leading-relaxed border-l-2 border-primary/20 pl-4 py-1.5 animate-fade-in-right bg-primary/5 rounded-r-lg",c.innerHTML=`
                ...
                <span class="text-sage-800 dark:text-slate-300 font-medium">${b}</span>
            `,p.insertBefore(c,p.firstChild)}};x()},re=s=>{s.innerHTML=`
    ...
    `,s.querySelectorAll(".nav-button").forEach(e=>{e.addEventListener("click",a=>{const m=a.currentTarget.dataset.screen;m&&window.router.navigate(m)})})},ie=s=>{let e=[null,null];const a=()=>{const{inventory:u,weather:g}=v.getState(),t=(()=>{if(!g)return{text:"Stable Conditions",icon:"sync",active:!1};const o=g.condition.toLowerCase();return o.includes("rain")?{text:"Water Infusion Active",icon:"water_drop",active:!0,bonus:"Higher Success Rate"}:o.includes("sun")||o.includes("clear")?{text:"Solar Hardening",icon:"sunny",active:!0,bonus:"Stronger Equipment"}:{text:"Standard Conditions",icon:g.icon,active:!1}})();s.innerHTML=`
<div class="bg-background-light dark:bg-background-dark font-display min-h-screen flex flex-col overflow-x-hidden text-slate-900 dark:text-slate-100 relative">
<div class="fixed inset-0 pointer-events-none opacity-40 z-0 bg-paper-texture mix-blend-multiply dark:mix-blend-overlay"></div>
<!-- Top Navigation -->
${T("workshop")}
...
<div class="flex items-center gap-2 px-3 py-1.5 rounded-xl ${t.active?"bg-amber-50 dark:bg-amber-900/30 border-amber-200":"bg-slate-50 dark:bg-slate-800 border-slate-200"} border text-xs font-bold transition-all">
<span class="material-symbols-outlined text-sm ${t.active?"text-amber-500":"text-slate-400"}">${t.icon}</span>
<span class="${t.active?"text-amber-700 dark:text-amber-400":"text-slate-500"}">${t.text} ${t.active?`(${t.bonus})`:""}</span>
</div>
</div>
</div>
<!-- Crafting Slots -->
...
<span class="text-xs font-bold text-slate-400 uppercase tracking-widest">${u.reduce((o,d)=>o+d.count,0)}/50 Slots</span>
</div>
<!-- Search/Filter -->
...
${u.map(o=>`
<div class="inventory-item aspect-square rounded-xl bg-slate-100 dark:bg-slate-800 border-2 border-transparent hover:border-primary/50 cursor-pointer transition-all flex items-center justify-center group relative p-1" data-id="${o.id}">
<span class="material-symbols-outlined text-primary text-2xl group-hover:scale-110 transition-transform">${o.icon}</span>
<span class="absolute bottom-1 right-1 bg-white/80 dark:bg-black/60 px-1 rounded text-[10px] font-bold">${o.count}</span>
...
`).join("")}
    ...
    `,z(s),m()},m=()=>{var u,g,l,t;s.querySelectorAll(".inventory-item").forEach(o=>{o.addEventListener("click",d=>{const r=d.currentTarget.dataset.id,i=v.getState().inventory.find(x=>x.id===r);i&&i.count>0&&(e[0]?e[1]||(e[1]=i):e[0]=i,a())})}),(u=document.getElementById("slot-0"))==null||u.addEventListener("click",()=>{e[0]=null,a()}),(g=document.getElementById("slot-1"))==null||g.addEventListener("click",()=>{e[1]=null,a()}),(l=document.getElementById("btn-clear"))==null||l.addEventListener("click",()=>{e=[null,null],a()}),(t=document.getElementById("btn-craft"))==null||t.addEventListener("click",()=>{if(e[0]&&e[1]){const{inventory:o,weather:d}=v.getState();e[0].id==="i1"&&e[1].id==="i2"?((d==null?void 0:d.condition.toLowerCase())||"").includes("rain")?alert("The rain infuses your Nectar! You crafted a 'Storm Nectar' (+50% Stamina)!"):alert(`Crafted successful! Used ${e[0].name} and ${e[1].name}.`):alert(`Crafted successful! Used ${e[0].name} and ${e[1].name}.`);const r=o.map(i=>{var f,k;let x=i.count;return i.id===((f=e[0])==null?void 0:f.id)&&x--,i.id===((k=e[1])==null?void 0:k.id)&&x--,{...i,count:x}});e=[null,null],v.setState({inventory:r}),a()}}),s.querySelectorAll(".nav-link, .nav-button").forEach(o=>{o.addEventListener("click",d=>{const r=d.currentTarget.dataset.screen;r&&window.router.navigate(r)})})};a()},le=s=>{window.location.hash==="#social"&&(s.innerHTML=`
    ...
    `,z(s))},oe=s=>{if(window.location.hash!=="#store")return;const e=[{id:"s1",name:"Sobre de Iniciación",price:500,type:"Card Pack",image:"https://images.pexels.com/photos/4060435/pexels-photo-4060435.jpeg?auto=compress&cs=tinysrgb&w=200",desc:"Contiene 3 pájaros comunes."},{id:"s2",name:"Néctar Premium",price:150,type:"Consumable",image:"https://images.pexels.com/photos/1013444/pexels-photo-1013444.jpeg?auto=compress&cs=tinysrgb&w=200",desc:"Restaura toda la estamina."},{id:"s3",name:"Prismáticos de Oro",price:2e3,type:"Equipment",image:"https://images.pexels.com/photos/4033230/pexels-photo-4033230.jpeg?auto=compress&cs=tinysrgb&w=200",desc:"Aumenta el rango de visión."},{id:"s4",name:"Caja Regalo",price:1e3,type:"Bundle",image:"https://images.pexels.com/photos/1579240/pexels-photo-1579240.jpeg?auto=compress&cs=tinysrgb&w=200",desc:"Regalo aleatorio de alto nivel."}];s.innerHTML=`
    <div class="bg-cream dark:bg-background-dark text-slate-900 dark:text-slate-100 font-display transition-colors duration-200 min-h-screen flex flex-col relative overflow-x-hidden">
        <div class="fixed inset-0 pointer-events-none opacity-40 z-0 bg-paper-texture mix-blend-multiply dark:mix-blend-overlay"></div>
        <div class="relative z-10 flex flex-col flex-grow w-full max-w-[1440px] mx-auto">
            ${T("store")}
                    ...
                    ${e.map(a=>`
                        <div class="bg-white/80 dark:bg-background-dark/80 backdrop-blur-md rounded-3xl overflow-hidden border border-sage-100 dark:border-sage-800 shadow-card hover:shadow-journal transition-all group animate-fade-in-up">
                            <div class="aspect-square overflow-hidden relative">
                                <img src="${a.image}" alt="${a.name}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                            ...
                            <div class="p-5 flex flex-col items-center text-center">
                                <h3 class="font-bold text-lg text-sage-800 dark:text-white mb-1">${a.name}</h3>
                                <p class="text-xs text-slate-500 dark:text-slate-400 mb-4 line-clamp-2">${a.desc}</p>
                                <div class="flex items-center gap-2 mb-4">
                                    <span class="material-symbols-outlined text-amber-500">monetization_on</span>
                                    <span class="text-2xl font-black text-sage-800 dark:text-white">${a.price}</span>
                                </div>
    ...
    `,z(s)},ne=s=>{var a;const e=m=>{m.preventDefault(),window.router.navigate("home")};s.innerHTML=`
    ...
    `,(a=document.getElementById("login-form"))==null||a.addEventListener("submit",e)},B=document.querySelector("#app"),E=new K("app");window.router=E;E.register("home",async()=>{se(B)});E.register("login",async()=>{ne(B)});E.register("expedition",async()=>{ae(B)});E.register("arena",async()=>{F(B)});E.register("album",async()=>{re(B)});E.register("workshop",async()=>{ie(B)});E.register("social",async()=>{le(B)});E.register("store",async()=>{oe(B)});E.init();
