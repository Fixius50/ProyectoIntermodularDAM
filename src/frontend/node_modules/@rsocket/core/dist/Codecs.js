...
function deserializeFrame(buffer) {
    ...
    offset += 2;
    const type = typeAndFlags >>> exports.FRAME_TYPE_OFFSET; // keep highest 6 bits
    const flags = typeAndFlags & exports.FLAGS_MASK; // keep lowest 10 bits
    switch (type) {
...
function serializeSetupFrame(frame) {
    const resumeTokenLength = frame.resumeToken != null ? frame.resumeToken.byteLength : 0;
...
function deserializeSetupFrame(buffer, streamId, flags) {
    ...
    offset += 1;
    const metadataMimeType = buffer.toString("ascii", offset, offset + metadataMimeTypeLength);
    offset += metadataMimeTypeLength;
    const dataMimeTypeLength = buffer.readUInt8(offset);
    offset += 1;
    const dataMimeType = buffer.toString("ascii", offset, offset + dataMimeTypeLength);
...
function serializeErrorFrame(frame) {
    const messageLength = frame.message != null ? Buffer.byteLength(frame.message, "utf8") : 0;
    const buffer = Buffer.allocUnsafe(FRAME_HEADER_SIZE + ERROR_FIXED_SIZE + messageLength);
    let offset = writeHeader(frame, buffer);
...
function serializeKeepAliveFrame(frame) {
    ...
    const buffer = Buffer.allocUnsafe(FRAME_HEADER_SIZE + KEEPALIVE_FIXED_SIZE + dataLength);
    let offset = writeHeader(frame, buffer);
...
function serializeLeaseFrame(frame) {
    ...
    const buffer = Buffer.allocUnsafe(FRAME_HEADER_SIZE + LEASE_FIXED_SIZE + metaLength);
    let offset = writeHeader(frame, buffer);
...
function serializeRequestManyFrame(frame) {
    const payloadLength = getPayloadLength(frame);
    const buffer = Buffer.allocUnsafe(FRAME_HEADER_SIZE + REQUEST_MANY_HEADER + payloadLength);
    let offset = writeHeader(frame, buffer);
...
function serializeResumeFrame(frame) {
    ...
    const buffer = Buffer.allocUnsafe(FRAME_HEADER_SIZE + RESUME_FIXED_SIZE + resumeTokenLength);
    let offset = writeHeader(frame, buffer);
    offset = buffer.writeUInt16BE(frame.majorVersion, offset);
    offset = buffer.writeUInt16BE(frame.minorVersion, offset);
    offset = buffer.writeUInt16BE(resumeTokenLength, offset);
...
function writeHeader(frame, buffer) {
    ...
    return buffer.writeUInt16BE((frame.type << exports.FRAME_TYPE_OFFSET) | (frame.flags & exports.FLAGS_MASK), offset);
}
/**
