...
class RequestChannelRequesterStream {
    constructor(payload, isComplete, receiver, fragmentSize, initialRequestN, leaseManager) {
    ...
    handleReady(streamId, stream) {
        ...
        if ((0, Fragmenter_1.isFragmentable)(this.payload, this.fragmentSize, Frames_1.FrameTypes.REQUEST_CHANNEL)) {
            for (const frame of (0, Fragmenter_1.fragmentWithRequestN)(streamId, this.payload, this.fragmentSize, Frames_1.FrameTypes.REQUEST_CHANNEL, this.initialRequestN, isCompleted)) {
    ...
    handle(frame) {
        ...
        switch (frameType) {
            case Frames_1.FrameTypes.PAYLOAD: {
                const hasComplete = Frames_1.Flags.hasComplete(frame.flags);
                const hasNext = Frames_1.Flags.hasNext(frame.flags);
    ...
    onNext(payload, isComplete) {
        ...
        if ((0, Fragmenter_1.isFragmentable)(payload, this.fragmentSize, Frames_1.FrameTypes.PAYLOAD)) {
            for (const frame of (0, Fragmenter_1.fragment)(this.streamId, payload, this.fragmentSize, Frames_1.FrameTypes.PAYLOAD, isComplete)) {
                this.stream.send(frame);
            }
        }
        else {
...
class RequestChannelResponderStream {
    constructor(streamId, stream, fragmentSize, handler, frame) {
        this.streamId = streamId;
        this.stream = stream;
        this.fragmentSize = fragmentSize;
    ...
    handle(frame) {
        ...
        switch (frameType) {
            ...
            case Frames_1.FrameTypes.CANCEL: {
                ...
                if (!inboundDone) {
                    const error = frameType === Frames_1.FrameTypes.CANCEL
    ...
    onError(error) {
        if (this.outboundDone) {
            console.warn(`Trying to error for the second time. ${error ? `Dropping error [${error}].` : ""}`);
            return;
        }
        const inboundDone = this.inboundDone;
        this.outboundDone = true;
    ...
    onNext(payload, isCompletion) {
        ...
        // TODO: add payload size validation
        if ((0, Fragmenter_1.isFragmentable)(payload, this.fragmentSize, Frames_1.FrameTypes.PAYLOAD)) {
            for (const frame of (0, Fragmenter_1.fragment)(this.streamId, payload, this.fragmentSize, Frames_1.FrameTypes.PAYLOAD, isCompletion)) {
                this.stream.send(frame);
            }
        }
        else {
